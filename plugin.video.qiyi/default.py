# -*- coding: utf-8 -*-
import xbmc, xbmcgui, xbmcplugin, xbmcaddon, urllib2, urllib, re, sys, zlib
import hashlib
import time
import simplejson
...
CHANNEL_LIST = [['电影', '1'], ['电视剧', '2'], ['纪录片', '3'], ['动漫', '4'], ['音乐', '5'], ['综艺', '6'], ['娱乐', '7'],
                ['旅游', '9'], ['片花', '10'], ['教育', '12'], ['时尚', '13']]
...
def progList(name, id, page, cat, area, year, order, paytype):
    ...
    url = 'http://list.iqiyi.com/www/' + id + '/' + c1 + '-' + c2 + '-' + c3 + '-' + c4 + '-------' + \
          paytype + '-' + '' + '--' + '24' + '-' + page + '-1-iqiyi--.html'

    currpage = int(page)
    link = GetHttpData(url)
    # print(link)
    match1 = re.compile('data-id="([0-9]+)"').findall(link)
    if len(match1) == 0:
        totalpages = 1
    else:
        totalpages = int(match1[len(match1) - 1])
    # print('total page %d'%(totalpages,))
    match = re.compile('<!-- 乐高标签 -->(.+?)<!-- 分类 end-->', re.DOTALL).findall(link)
    if match:
        listpage = match[0]
    else:
        listpage = ''
    match = re.compile('<ul class="qy-mod-ul">(.+?)</ul>', re.DOTALL).findall(link)
    if match:
        # match = re.compile('<li([^>]*?"albumId"[^>]*?>.+?)</li>', re.DOTALL).findall(match[0])
    ...
    u = sys.argv[0] + "?mode=4&name=" + urllib.quote_plus(name) + "&id=" + urllib.quote_plus(
        id) + "&cat=" + urllib.quote_plus(cat) + "&area=" + urllib.quote_plus(area) + "&year=" + urllib.quote_plus(
        year) + "&order=" + order + "&paytype=" + urllib.quote_plus(paytype) + "&page=" + urllib.quote_plus(listpage)
    xbmcplugin.addDirectoryItem(int(sys.argv[1]), u, li, True, totalItems)
    for i in range(0, len(match)):
        # print(match[i])
        p_name = re.compile('title="(.+?)"').findall(match[i])[0]
        p_thumb = "http:%s" % (re.compile('src\s*=\s*"(.+?)"').findall(match[i])[0],)
        # p_id  = re.compile('data-qidanadd-albumid="(\d+)"').search(match[i]).group(1)
        p_id = "http:%s" % (re.compile('href="([^"]*)"').search(match[i]).group(1),)
        # print("name:%s\nthumb:%s\nid:%s" % (p_name, p_thumb, p_id,))
        p_episode = False
        p_name1 = p_name
        if  "albumId" in match[i]:
            re_sul = re.compile('<span class="qy-mod-label">([^<]+?)</span>').search(match[i])
            if bool(re_sul):
                msg = re_sul.group(1).strip()
                p_name1 = p_name + '（' + msg + '）'
                p_episode = True
            else:
                continue
        ...
        if p_episode:
            mode = 2
            isdir = True
            p_id = re.compile('"albumId"\:"(\d+?)"').search(match[i]).group(1)
        else:
            mode = 3
            isdir = False
        p_rating = 0
        ...
        li.setInfo(type="Video", infoLabels={"Title": p_name, "Director": p_director, "Genre": p_genre, "Plot": p_plot,
                                             "Cast": p_cast, "Rating": p_rating})
    ...
    if currpage > 1:
        ...
        u = sys.argv[0] + "?mode=1&name=" + urllib.quote_plus(name) + "&id=" + urllib.quote_plus(
            id) + "&cat=" + urllib.quote_plus(cat) + "&area=" + urllib.quote_plus(area) + "&year=" + urllib.quote_plus(
            year) + "&order=" + order + "&page=" + urllib.quote_plus(str(currpage - 1)) + "&paytype=" + paytype
        xbmcplugin.addDirectoryItem(int(sys.argv[1]), u, li, True, totalItems)
...
def seriesList(name, id, thumb, page):
    ...
    if albumType in (1, 6, 9, 12, 13) and sourceId <> 0:
        url = 'http://cache.video.qiyi.com/jp/sdvlst/%d/%d/?categoryId=%d&sourceId=%d' % (
        albumType, sourceId, albumType, sourceId)
        link = GetHttpData(url)
        data = link[link.find('=') + 1:]
        json_response = simplejson.loads(data)
        ...
        for item in json_response['data']:
            ...
            li.setInfo(type="Video",
                       infoLabels={"Title": p_name, "Director": p_director, "Cast": p_cast, "Plot": p_plot,
                                   "Year": p_year})
            u = sys.argv[0] + "?mode=3&name=" + urllib.quote_plus(p_name) + "&id=" + urllib.quote_plus(
                p_id) + "&thumb=" + urllib.quote_plus(p_thumb)
        ...
        if currpage > 1:
            li = xbmcgui.ListItem('上一页')
            u = sys.argv[0] + "?mode=2&name=" + urllib.quote_plus(name) + "&id=" + urllib.quote_plus(
                id) + "&thumb=" + urllib.quote_plus(thumb) + "&page=" + urllib.quote_plus(str(currpage - 1))
            xbmcplugin.addDirectoryItem(int(sys.argv[1]), u, li, True, totalItems)
        if currpage < totalpages:
            li = xbmcgui.ListItem('下一页')
            u = sys.argv[0] + "?mode=2&name=" + urllib.quote_plus(name) + "&id=" + urllib.quote_plus(
    ...
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
# seriesList(None,'245611201','http://pic9.iqiyipic.com/image/20190816/20/0a/a_100335889_m_601_m6_260_360.jpg','1')
...
def selResolution(items):
    ...
    for i in range(0, len(items)):
        if items[i] == 96:
            ...
            ratelist.append([5, '标清 H265', i])
        elif items[i] == 3:
            ratelist.append([4, '超清', i])
        elif items[i] in (4, 14):
            ratelist.append([3, '720P', i])
        elif items[i] == 17:
            ratelist.append([3, '720p H265', i])
        elif items[i] in (5, 12, 18):
            ratelist.append([2, '1080P', i])
        elif items[i] in (10, 19):
...
### http://www.iqiyi.com/v_19rrj6vhvs.html http://pic6.iqiyipic.com/image/20190912/de/06/v_50001553_m_601_m9_260_360.jpg
...
# PlayVideo(None,'http://www.iqiyi.com/v_19rrj6vhvs.html','http://pic6.iqiyipic.com/image/20190912/de/06/v_50001553_m_601_m9_260_360.jpg')
